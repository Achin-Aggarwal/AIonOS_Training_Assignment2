...
def parse_install_request_with_llm(text: str) -> List[Dict[str, str]]:
    ...
    try:
        ...
        # Enhanced system prompt with better examples and instructions
        system_prompt = """
    ...
    # Enhanced fallback to regex parsing
...
def parse_install_request_regex(text: str) -> List[Dict[str, str]]:
    ...
    # Pre-process the text to handle common separators and patterns
    text = text.replace(' and ', ', ').replace(' & ', ', ').replace(';', ',')
    
    # First, check if this looks like an install request
    ...
    # Remove the install command words to focus on software names
    ...
    # If no commas, try other separators
    if len(parts) <= 1:
    ...
    for part in parts:
        ...
        # Remove version info to get clean software name
        if version_match:
            part = part.replace(version_match.group(0), '').strip()
        
        # Clean up the software name
    ...
    # If regex didn't find multiple software, try a different approach
    if len(potential_software) <= 1:
        # Try to split by known software names
        known_software = [
        ...
        for software in known_software:
            if software in text_lower:
                # Extract version if present near this software
...
def clean_software_name(name: str) -> str:
    ...
    # Remove common words that shouldn't be part of software names
    ...
    # Handle common software name corrections and expansions
    name_corrections = {
    ...
    # If no exact match, return title cased version
...
def approval_router(state):
    ...
    # Check if any software is approved
                    ...
                    for i, ticket_id in enumerate(result["ticket_ids"]):
                        ...
                        if approval_status == "approved":
                            ...
                            for j, step in enumerate(steps):
                                ...
                                time.sleep(0.5)  # Faster for multiple installations
...
if os.path.exists(EXCEL_FILE):
    ...
    if not df.empty:
        ...
        # Show more recent tickets for better visibility
...
# Enhanced auto-refresh for multiple pending tickets
if os.path.exists(EXCEL_FILE):
    df = pd.read_excel(EXCEL_FILE)
